...
def main(graph): 
	...
	for scenario in graph.getSubGraphs():
		print (scenario.getName())
		for s in scenario.getSubGraphs():
			name = s.getName()
			for n in s.getNodes():
				if manager[n] == True:
					incoming = 0
					mS = 0
					for e in s.getInEdges(n):
						incoming += 1
						mS += intimacy[e]
					outgoing = 0
					for e2 in s.getOutEdges(n):
						outgoing += 1
			print (name + ': inEdges = ' + str(incoming) + ', mS = ' + str(mS) + ', outEdges 0 ' + str(outgoing))
